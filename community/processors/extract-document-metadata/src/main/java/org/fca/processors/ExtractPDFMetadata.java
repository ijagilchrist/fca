...
public class ExtractPDFMetadata extends ExtractTikaMetadata {

    public ExtractPDFMetadata() {
    ...
    public List<MicroFormat> process(Context context, MicroFormat microFormat) {
        ...
        if (this.isPDFDocument(identifiedObject.getObjectMimetype())) {
    ...
    private void extractMetadata(TikaMetadata tikaMetadata, DocumentMetadata.Builder documentMetadataBuilder) {
        ...
        documentMetadataBuilder.setTitle(this.getLongest(metadata, "","pdf:docinfo:title", "dc:title","title"));

    }

    protected boolean isPDFDocument(String mimetype) {
